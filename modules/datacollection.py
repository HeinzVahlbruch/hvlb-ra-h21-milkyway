...
class SalsaFITS(Observation):
    ...
    def _read_data(self):
        ...
        v = (- self._c_kms * (f - crval1) / crval1 - vLSR) / 1000
...
class Plots():
    ...
    def peaks_plots_analysis(obs_dic, columns, peak_finder_params):
        long, velocities, max_velocities, instrument, telescope, obs_date = [],[],[],[],[],[]
        pir = pic = 0
        ...
        peak_results = pd.DataFrame({'longitude':long, 'velocity':velocities, 'max_velocity':max_velocities, 'telescope':telescope, 'instrument':instrument, 'date':obs_date})
        return (peak_results)
